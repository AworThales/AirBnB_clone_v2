...
class TestConsole(unittest.TestCase):
    ...
    def test_pep8_console(self):
        ...
        point = style.check_files(["console.py"])
        self.assertEqual(point.total_errors, 0, 'fix Pep8')
    ...
    def test_update(self):
        ...
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd("all User")
            obj = f.getvalue()
        an_id = obj[obj.find('(')+1:obj.find(')')]
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd("update User " + an_id)
            self.assertEqual(
                "** attribute name missing **\n", f.getvalue())
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd("update User " + an_id + " Name")
            self.assertEqual(
                "** value missing **\n", f.getvalue())
    ...
    def test_update(self):
        ...
        an_id = obj[obj.find('(')+1:obj.find(')')]
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd("User.update(" + an_id + ")")
            self.assertNotEqual("** attribute name missing **",
                                f.getvalue())
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd("User.update(" + an_id + ", name)")
